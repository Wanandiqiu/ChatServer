...
## v0.4.4 — 应用层心跳（2026-05-30）

校园内测 **M1（R2）** 子集：连接保活与空闲踢线。

### 能力

| 类别 | 状态 | 说明 |
|------|------|------|
| 心跳请求/应答 | ✅ | `HEARTBEAT_MSG` / `_ACK` (35/36)，未登录也可发 |
| 空闲超时 | ✅ | 任意应用帧或心跳刷新；**90s** 无活动则断开并 `onDisconnect` |
| 客户端建议间隔 | ✅ | 响应含 `recommended_interval_sec=30` |
| CLI | ✅ | 主循环每 **25s** 自动发心跳 |
...
| MsgType | 说明 |
|---------|------|
| `HEARTBEAT_MSG` / `_ACK` (35/36) | `HeartbeatReq` 空；`HeartbeatRsp` 含建议间隔与 `server_idle_timeout_sec` |

### 验收

1. 登录后静置 >90s 不发任何操作 → 连接被服务端关闭
2. 登录后保持 CLI 打开（自动心跳）→ 连接保持
3. 可用抓包或日志确认约 25s 一次 `HEARTBEAT_MSG`
